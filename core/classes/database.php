...
class database extends general
{
    ...
    public function write_log($query,$time){
        $myFile = "log.txt";
        $fh = fopen($myFile, 'a') or die("can't open file");
        $string = $time.'                    '.$query."\r\n";
        fwrite($fh, $string) or die("error writing");
        fclose($fh);
    ...
    */
    public function select($table, $rows = '*', $where = null, $order = null, $limit = "")
    {
        ...
        if($limit != "")
            $q .= ' LIMIT '.$limit;
        $query = @mysql_query($q);
        $this->write_log($q,date("m/d/y G.i:s", time()));
        if($query)
        {
    ...
    */
    public function insert($table,$values,$rows = null)
    {
        if($this->tableExists($table))
        {
            ...
            $this->write_log($insert,date("m/d/y G.i:s<br>", time()));
            if($ins)
            {
                return true;
            }
    ...
    */
    public function delete($table,$where = null)
    {
        if($this->tableExists($table))
        {
            ...
            $this->write_log($delete,date("m/d/y G.i:s<br>", time()));
            if($del)
            {
                return true;
            }
    ...
    public function update($table,$rows,$where)
    {
        if($this->tableExists($table))
        {
            ...
            $this->write_log($update,date("m/d/y G.i:s<br>", time()));
            if($query)
            {
                return true;
            }
